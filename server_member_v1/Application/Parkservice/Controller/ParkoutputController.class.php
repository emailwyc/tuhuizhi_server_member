            ...
            if ($sign==$othersign){
                ...
                if (null != $classname){
                    ...
                    if (is_array($result)){//如果是json，必然是正确的数据，类文件里面必须处理正确
                        ...
                        /*
                        if(!isset($result['data']['IntValue']) && !isset($result['data']['0'])){
        ...
        if (null==$carno || null==$key_admin || null==$othersign){
            ...
            if ($sign==$othersign){
                ...
                if (null != $classname){
                    ...
                    if (is_array($result)){//如果是json，必然是正确的数据，类文件里面必须处理正确
                        //========如果积分字段为0或者未返回,需要调取系统积分比率进行换算;
                        //========1130代表已经支付
                        if(empty($result['data']['IntValue'])){
                            $scoreInfo = $this->GetOneAmindefault($admininto['pre_table'],$key_admin,"score");
                            $result['data']['IntValue'] = @(int)$scoreInfo['function_name']*($result['data']['MoneyValue']/100);
                            $result['data']['IntValue'] = number_format($result['data']['IntValue'], 2, '.', '');
                        }
...
//            }
            ...
            if ($sign==$othersign){
                ...
                if (null != $classname){
                    ...
                    //======把积分比率传过去进行换算;
                    $scoreInfo = $this->GetOneAmindefault($admininto['pre_table'],$key_admin,"score");
                    $obj->scorepv = @(int)$scoreInfo['function_name'];
