...
{
    // 未来中心停车系统配置
    protected $url = 'http://182.150.41.12:7799/bosiny/wechat';//http://h0000k.imwork.net:57799/bosiny/wechat';
    protected $post = 9749;
    ...
    public function searchcar($carno,$sing_key,$key_admin,$page,$lines){
        ...
        if (!empty($array['body'])){
            $time = strtotime($array['body'][0]['indate'].$array['body'][0]['intime']);
            
            $re_data['BeginTime'] = date('Y-m-d H:i:s', $time);
