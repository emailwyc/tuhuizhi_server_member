...
//        }
        ...
        if ('' != $erpserviceclass && 'java' != $erpserviceclass){
            if ($action == 'get_exchange_list'){
                $action = 'getExchangeList';
            }
            if ($action == 'prize_exchange'){
                $action = 'prizeExchange';
            }
            if ($action == 'prize_return'){
                $action = 'prizeReturn';
            }
            if ($action == 'prize_list'){
                $action = 'prizeList';
            }

            $adminInfo = $this->getMerchant($key_admin);
            $data = ErpCommonService::$action($params, $adminInfo, $classname);
            returnjson($data);
