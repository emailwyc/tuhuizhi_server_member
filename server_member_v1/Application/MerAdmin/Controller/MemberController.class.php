...
//         print_r($res);die;
        if($res['code']==200){
            if($this->ukey == '202cb962ac59075b964b07152d234b70' || $this->ukey == '75458833a43dc64df5069e03bdad1ec5'){
                $str[]="用户名,微信号,会员卡号,身份证号,手机号,性别,生日,等级,职业,星座,住址,备注,状态,积分,注册时间";
            }else{
            ...
            foreach($res['data']['data'] as $k=>$v){
                ...
                if($this->ukey == '202cb962ac59075b964b07152d234b70' || $this->ukey == '75458833a43dc64df5069e03bdad1ec5'){
                    $str[]=$v['usermember'].",".$v['wechat'].",".$v['cardno'].",".$v['idnumber'].",".$v['mobile'].",".$sex.",".date('Y-m-d',$v['birthday']).",".$v['level'].",".$v['career'].",".$v['star'].",".$v['address'].",".$v['remark'].",".$status.",".$v['score_num'].",".$v['getcarddate'];
                }else{
    ...
    public function MemberLevelList(){
	    ...
	    if($arr){
	        $msg['code']=200;
	        $msg['data']=json_decode($arr['function_name'], true);;
	    }else{
	        $msg['code']=102;
	    }
...
//                 echo $db->_sql();die;
    ...
    public function member_actions($status){
        ...
        if (in_array('', $params)){
            ...
            $map['_logic']='and';
            if($status== 'export'){
                $sel=$db->field('id,cardno,usermember,idnumber,sex,getcarddate,expirationdate,phone,mobile,birthday,is_del as status,score_num,headerimg,address,career,wechat,star,remark,openid,email,level,province,city,district')->where($map)->order('id desc')->select();
            }else{
                $sel=$db->field('id,cardno,usermember,idnumber,sex,getcarddate,expirationdate,phone,mobile,birthday,is_del as status,score_num,headerimg,address,career,wechat,star,remark,openid,email,level,province,city,district')->where($map)->order('id desc')->limit($start,$params['lines'])->select();
            }
//             echo $db->_sql();die;
            if (false != $sel){
        
                $return=$this->GetOneAmindefault($this->admin_arr['pre_table'], $params['key_admin'], 'member_level');
                $arr_level=json_decode($return['function_name'],true);
                foreach($arr_level as $k=>$v){
                    $arr[$v['code']]=$v['level'];
                }
                ...
                foreach($default_arr as $k=>$v){
                        if($v['fromtype'] != 'select'){
                            ...
                            foreach($v['option'] as $ks=>$vs){
                                if($vs['type'] == 'level'){
                                    $default_arr[$k]['option'][$ks]['value']=$level;
                                }
...
// 	        //}
	...
	public function reason_list(){
	    ...
	    if($this->ukey == '75458833a43dc64df5069e03bdad1ec5'){
	       unset($arr[6]);   
	       unset($arr[9]);
	       unset($arr[12]);
	    }
