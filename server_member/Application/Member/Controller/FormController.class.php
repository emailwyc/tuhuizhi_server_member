...
//        $sel=$db->field('total_form.id,isrequired,placeholder,sub,value,minlength,maxlength,content,content_key,content_type,function_name')->join($join)->where(array('isenable'=>1))->select();
        ...
        $create_url=$this->GetOneAmindefault($admininfo['pre_table'], $this->ukey, 'createsuccessurl');
        $jump_url=$create_url['function_name']?$create_url['function_name']:'';
        
        $data = array('code' => '200','data'=>array('url'=>$jump_url), 'msg' => 'success');
        returnjson($data, $this->returnstyle, $this->callback);
    }
...
//        returnjson(json_decode($json, true), $this->returnstyle, $this->callback);
        ...
        $join=' join `total_form` on `form_key_id` = `total_form`.`id`';
//        $sel=$db->join($join)->where(array('isenable'=>1))->select();
        $sel=$db->field('total_form.id,isrequired,placeholder,sub,value,minlength,maxlength,content,content_key,content_type,function_name,sort')->join($join)->where(array('isenable'=>1,'type'=>0))->order('sort asc')->select();
