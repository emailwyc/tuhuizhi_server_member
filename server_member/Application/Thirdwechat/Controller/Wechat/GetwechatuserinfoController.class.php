                ...
                if (null == $key_admin || null == $sign || null == $page || 1>$page){
                    ...
                    if (1<count($sel)){
                        ...
                        if ($versign==$sign){
                            ...
                            $dbwechat=M('wechat_openid',$sel[0]['pre_table'],'DB_CONFIG1');
...
//         }
    ...
    public function GetAllUserInfo(){
        ...
        //查询微信授权表
        $db=M('third_authorizer_info','total_','DB_CONFIG1');
        $wechatlist=$db->join(' `total_admin` on `total_admin`.`wechat_appid` = `total_third_authorizer_info`.`appid`')->field('authorization_info,appid,pre_table,nick_name')->select();
        $wechat='';
        //查询符合条件的appid
        foreach ($wechatlist as $key => $val){
            $authorization=explode(',', $val['authorization_info']);
        ...
        foreach ($wechat as $k => $v){
            ...
            if (null == $find){
                ...
                echo $v['appid'].'  was get.'.'--stop:'.date('Y-m-d H:i:s')."\r\n\r\n\r\n";
                continue;
            }
        }
    }
