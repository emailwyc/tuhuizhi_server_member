...
class RiYueParkController extends Controller implements ParkinterfaceController
{
    /**
     * 获取剩余车位数
    ...
    protected function carno_num($floor){
        $url = 'http://221.182.179.102:9999/ZRWS.asmx/FindCarInfoByFloor';
        $params['floor']=$floor;
        $params['carUse']=0;
    ...
    public function searchcar($carno,$sing_key,$key_admin,$page,$lines){
        ...
        $url = 'http://221.182.179.102:9999/ZRWS.asmx/FindCarInfo';
...
//         $param['key_admin'] = $key_admin;
        $url = 'http://221.182.179.102:9999/ZRWS.asmx/FindCarInfoAll';
        $curl_re = http($url);
        if(!is_json($curl_re)){
            return false;
        }
