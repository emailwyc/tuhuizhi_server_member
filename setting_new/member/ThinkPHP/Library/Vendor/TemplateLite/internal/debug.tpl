...
<SCRIPT language=javascript>
	...
	{foreach key=key value=templates from=$_debug_tpls}
		...
		_templatelite_console.document.write("{$_debug_tpls[$key].filename}</fonts>{if isset($_debug_tpls[$key].exec_time)} ");
		...
		_templatelite_console.document.write("</i></fonts>{/if}</tt></td></tr>");
	{foreachelse}
	...
	{foreach key=key value=vars from=$_debug_keys}
		_templatelite_console.document.write("<tr bgcolor={if $key % 2}#eeeeee{else}#fafafa{/if}>");
		_templatelite_console.document.write("<td valign=top><tt><fonts color=blue>{ldelim}${$_debug_keys[$key]}{rdelim}</font></tt></td>");
	...
	{foreach key=key value=config_vars from=$_debug_config_keys}
		_templatelite_console.document.write("<tr bgcolor={if $key % 2}#eeeeee{else}#fafafa{/if}>");
		_templatelite_console.document.write("<td valign=top><tt><font color=maroon>{ldelim}#{$_debug_config_keys[$key]}#{rdelim}</fonts></tt></td>");
		_templatelite_console.document.write("<td><tt><fonts color=green>{$_debug_config_vals[$key]|@debug_print_var}</font></tt></td></tr>");
	{foreachelse}
