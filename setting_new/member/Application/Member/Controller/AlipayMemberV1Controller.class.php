...
class AlipayMemberController extends JaleelController {
    ...
    public function bindCard() {
        $mobile = I('mobile');
        $userid = I('userid');
        writeOperationLog(array('bind card para' => 'mobile:'.$mobile.',userid:'.$userid.',key_admin:'.$this->ukey),'jaleel_logs');
        ...
        $aliCardInfo = $this->registerAliCardno($mer_re,$uinfo['data']['cardno'], $userid, $uinfo['data']['cardtype']);
    ...
    public function cardIsUpgrade() {
        ...
        if(isset($scoreArr[$cardtype]) && false) {
            $url = C('DOMAIN') . '/ISV/Alipay/Ants/creditScoreBrief';
            $data = array(
                'key_admin' => $this->ukey,
    ...
    public function showCard() {
        ...
        // 查询会员信息
        $uinfo = $this->getUserCardByOpenId($mer_re['pre_table'], $this->userucid);
...
//        echo $curl_re;
    ...
    //注册联名会员卡
    protected function registerAliCardno($mer, $cardno, $userid,$grade)
    {
        $tpl=$this->GetOneAmindefault($mer['pre_table'], $mer['ukey'], 'alipay_card_tpl');
        $tplid = json_decode($tpl['function_name'], true);
        if ($tplid) {
            $tplid = json_decode($tpl['function_name'], true);
            $url = C('DOMAIN') . '/ISV/Alipay/MarketCard/openUserCard';
