...
//        }
    ...
    public function GetOneMessage()
    {
        ...
        $find['url'] = htmlspecialchars_decode($find['url']);
        $find['picurl'] = htmlspecialchars_decode($find['picurl']);
        if ($find){
...
//            }
            $this->redis->set('wechat:'.$message_event_type.':message:type:content:' . $admininfo['wechat_appid'], json_encode($find));//把所有数据存入redis
        }else{
  ...
  PRIMARY KEY (`id`)
    ...
    private function TemplateDbArray($templateid, $patams)
    {
        ...
        $array=array(
            ...
            'url'=>htmlspecialchars_decode($patams['url'])
        );

        foreach ($templatekeys['keys'] as $key => $val){
            $data[$val]=array('value'=>$patams[$val],'color'=>'#173177');
            $params['keys'][$val]=htmlspecialchars_decode($patams[$val]);
        }
