...
class Oauth2Controller extends WechatcommonController{
...
//         }
        ...
        $backurl=$hosttype.'://'.$_SERVER['HTTP_HOST'].U('/Thirdwechat/Wechat/Oauth2/getuserinfo/jumpurl/'.$jumpurl.'/scope/'.$scope,'','');
    ...
    private function get_access_token($appid,$code){
        ...
        if (is_json($return)){
            ...
            if (array_key_exists('access_token',$array)){
                ...
                $redis->set('wechat:mp:'.$appid.$array['openid'].'refresh_token',$array['refresh_token']);//设置此公众号的此openid的刷新token
        ...
        if ('' != $_GET['key_admin'] && !$_GET['code']){//根据key_admin获取appid，先读redis，如果没有，则读库
            ...
            if ('' == $appid){
                ...
                if (null != $find){
                    ...
                    $redis->set('wechat:'.$keyadmin.':admin',$find);
...
//                        echo 'Location:'.$jumpurl.'?openid='.$access_token;die;
