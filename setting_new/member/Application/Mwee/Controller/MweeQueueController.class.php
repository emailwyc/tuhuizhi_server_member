...
class MweeQueueController extends CommonController
{
    ...
    public function shopQueue()
    {
        ...
        $sid = CommonMwee::poinumberToSid($sid, $admininfo);
        if (false === $sid) {
        ...
        $array = array(
            'token'=>CommonMwee::$apptoken,//$this->mweeinfo['apptoken'],
            'sid'=>$sid,
            't'=>time()
        );
        $sn = CommonMwee::CreateSn($this->admininfo, $array);
        $array['sn'] = $sn;
    ...
    public function myQueue()
    {
        ...
        if (empty($params['orderid']) || empty($params['serialid'])) {
            if (!empty($params['sid'])) {//如果传了商铺的id，则查询用户在这个商铺的一条信息
                ...
                $db = M('mwee', $admininfo['pre_table']);
                $find = $db->where(array('sid'=>$sid,'userucid'=>$params['uid']))->order('id desc')->find();
                if ($find) {
        ...
        $sn = CommonMwee::CreateSn($this->admininfo, $array);//$this->mweeobj->CreateSn($this->admininfo, $array);
        $array['sn'] = $sn;
        $queue = http($url, $array, 'POST');
        if (is_json($queue)){
            $queuearr = json_decode($queue, true);
