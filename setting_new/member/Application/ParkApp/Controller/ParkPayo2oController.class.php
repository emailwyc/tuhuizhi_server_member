...
class ParkPayo2oController extends CommonController
{
    protected $url_hzt;
    protected $url_djy;
    ...
    public function _initialize()
    {
        ...
        $def_re = $this->GetOneAmindefault($mer_chant['pre_table'], $this->ukey, 'other_mer_config');
        if($def_re['function_name']){
            $merConf = json_decode($def_re['function_name'],true);
            //开始验证返回，（等联调的时候加返回）
        }
        */
    }
...
//            $total_fee = 100;
    ...
    public function checkIsMem(){
        // 查询会员信息
        $uinfo = $this->getUserCardByOpenId($mer_chant['pre_table'], $this->user_openid);
        $isMem = !empty($uinfo)?1:0;
        $data = array('code' =>200, 'data' =>array("isMem"=>$isMem));
        returnjson($data, $this->returnstyle, $this->callback);
    }
...
//            $this->createOrder($order, $carno, $total_fee, 0, $order_no, $begintime, $endtime, $client_orderno, $pay_fee, $discountFee, $freeTimeFee, $lowPriceFee, $freeTime);
        ...
        if ($pay_fee > 0) {
            ...
            $post_arr['attach'] = urlencode(json_encode(array('carNo' => $carno, 'key_admin' => $this->ukey, 'payType' => 2, 'amount' => $total_fee, 'discountfee' => $discountFee, 'freetimefee' => $freeTimeFee, 'payfee' => $pay_fee, 'client_orderno' => $client_orderno, 'order_no' => $order_no, 'use_freetime' => $use_freetime, 'openid' => $this->user_openid)));
            $post_arr['attach_transmit_tag'] = 'N';
            $post_arr['notify_url'] = C('DOMAIN') . "/ParkApp/ParkPayo2o/confirmPay";
            $post_arr['body'] = '停车支付';
