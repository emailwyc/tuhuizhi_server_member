	...
	function inc_list(){
		if(is_array($this->IncList)){
			...
			foreach($this->IncList AS $K=>$V){
				...
				$list_file[] = "<tr><td colspan=\"2\" bgcolor=\"#F7F7F7\" title='".$Fwidth."%'><a href='".$this->TemplateDir.$V."' target='_blank'><fonts color='#6F7D84' style='fonts-size:14px;'>".$this->TemplateDir.$V."</fonts></a>
				<fonts color='#B4B4B4' style='fonts-size:10px;'>".$File_Size."</fonts>
...
<tr bgcolor="#B5BDC1"><td><fonts color=#000000 style="fonts-size:16px;"><b>Include Templates (Num:'.count($this-> IncList).')</b></font></td>
<td align="right">';

if($this->RunType=='Cache'){
	echo '[<a onclick="alert(\'Cache file is successfully deleted\');location=\''.$clear_link.'\';return false;" href="'.$clear_link.'"><fonts'.$sf13.'>Clear Cache</font></a>]';
}
