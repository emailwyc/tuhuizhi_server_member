...
//        $this->type=I('type');
        ...
        $this->userappid = I('userappid');
//        $this->userucid = $this->user_openid ? $this->user_openid : ($this->user_userid ? $this->user_userid : $use);//全部将微信或支付宝的用户id转为统一的id，（user-client-id）

        if ($this->user_openid){
            $this->from='wechat';
            $this->userucid = $this->user_openid;
        }elseif ($this->user_userid){
            $this->from= 'alipay';
            $this->userucid = $this->user_userid;
        }elseif ($this->userappid){
            $this->from= 'app';
            $this->userucid = $this->userappid;
        }else{
...
//            $this->interfacelog();
  ...
  PRIMARY KEY (`id`)
     ...
     * 获取商户权限或栏目列表
     * @param $admin_id
     * @param int $showStatus| 1查询栏目列表，2查询所有勾选的接口权限
     * @return bool
     */
    protected function getAuthIds($admin_id, $showStatus = 1) {
        ...
        $res = $db->where($where)->join($auth.' on '.$auth.'.id='.$pre.'.auth_id')->order($pre.'.auth_id asc')->select();//,'show_status'=>1
        ...
        if(empty($return_arr)){
            ...
            $return_data['check_auth']=json_encode($return_arr);
            return $return_data;
        }
    }
	...
	protected function Auth_Admin($admin_id){
		$sel=$this->getAuthIds($admin_id, 2);
...
//        echo $logpath;
    ...
    //参数检查
    protected function issetCheck($params,$key_arr) {
