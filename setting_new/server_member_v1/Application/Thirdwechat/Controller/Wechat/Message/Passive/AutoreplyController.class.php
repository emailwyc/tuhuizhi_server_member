...
use Thirdwechat\Service\OpenWechat\Message\AutoReplayEventMessageService;
use Thirdwechat\Service\OpenWechat\Message\AutoReplayInputMessageService;
...
//                echostr('success');
//            }
            $data = AutoReplayInputMessageService::replayMessage($data, $get_data, $type, $appidInfo);
            if (false !== $data) {
                echo $data;
            }else{
                echostr('success');
            ...
            exit;
//            $this->textMessage($data, $get_data, $type, $appidInfo);exit;
        }
...
//            $appidInfo = $this->getAppidKeyAdmin($get_data['appid']);
            ...
            switch ($type){
                case 'text':
                    ...
                    exit;
//                    $array=$this->textMessage($data, $get_data, $type, $appidInfo);
                    break;
                case 'image':
            ...
            if ('subscribe' == $data['Event']){//如果事件是关注事件
                ...
                $this->EventReplayMessage($data, $get_data, $data['Event'], $appidInfo);
     ...
     * 关注、进入公众号
     */
    private function EventReplayMessage($data, $get_data, $eventtype, $appidInfo)
    {
        //判断此appid是否有发送关注推送的需求
        $is_subscribe_message=$this->redis->get('wechat:'.$eventtype.':ismessage:'.$get_data['appid']);
        //wechat:subscribe:ismessage:wxf3a057928b881466
        if ('yes' == $is_subscribe_message){
            ...
            //wechat:subscribe:message:type:wxf3a057928b881466
            if ('template' ==$messagtype){//模板消息
                ...
                unset($a);
            }elseif ('text' == $messagtype || 'news' == $messagtype){//文本或图文消息消息、音乐、视频、图片等图文消息用客服消息
                ...
                //wechat:news:todaymessageissended:wxf3a057928b881466location:oWm-rt2OE-JtS9JSxlldzjpV1V7M
                if ($rediskey == false){
//                    if ($get_data['appid'] == 'wxf3a057928b881466'){//此判断调试用
                        $encryptXml = AutoReplayEventMessageService::eventMessage($data['ToUserName'], $messagtype, $data['FromUserName'], $appidInfo, $get_data['appid'], $eventtype, $get_data);
                        if ($encryptXml != false){
                            echo $encryptXml;
...
//        writeOperationLog(array('data'=>$data, 'get_data'=>$get_data, 'eventtype'=>$eventtype,'xml'=>$xml), 'eventtest');
     ...
     */
    private function textMessage($data, $get_data, $type, $appidInfo)
    {
        $data = AutoReplayInputMessageService::replayMessage($data, $get_data, $type, $appidInfo);
        if (false !== $data) {
            echo $data;
