...
class EventsController extends ThirdwechatcommonController{
    ...
    public function mpEvent(){
        ...
        writeOperationLog($array,'mpEvent');
...
//         //ticket@@@HND9m0OqRhK-dPWsL2O4P3XfNOk-7dl_A9bXc6aXSg6ELLEYIl1fIxbuOO9XyYxXGer23_viW1XHdd1UR_vIuw
        ...
        writeOperationLog(array('xml_encrpty'=>$postData,'xml_json'=>json_encode($component_verify_ticket),'msg_signature'=>$msgSignature,'timestamp'=>$timestamp,'nonce'=>$nonce),'authEvents');
        ...
        if (0==$component_verify_ticket['code']){
            ...
            if ('component_verify_ticket'==$component_verify_ticket['infotype']){//每十分钟推送ticket事件
                //如果解密成功则设置ticket
                $rediss->set('wechat:component_verify_ticket',$component_verify_ticket['component_verify_ticket']);//保存ticket，因为微信每隔十分钟请求一次，所以无所谓设置时长
                //$rediss->close();
                F('wechat:component_verify_ticket', $component_verify_ticket['component_verify_ticket']);//存文件不行，存在多台服务器的问题
                echo 'success';
     ...
     * @return mixed
     */
    private function getWechatComponentVerifyTicket()
    {
        $data = $this->redis->get('wechat:component_verify_ticket');
        if (!$data) {
            $data = F('wechat:component_verify_ticket');
        }
        return $data;
    }
    ...
    private function get_component_access_token($component_verify_ticket){
        ...
        if (is_json($token)){
            ...
            if (!array_key_exists('errcode',$token)){//如果没有errorcode
                $redis = new RedisController();
                $rediss=$redis->connectredis();
                $rediss->set('wechat:component_access_token',$token['component_access_token'],array('ex'=>$token['expires_in']-120));
    ...
    public function get_pre_auth_code(){
        $redis = new RedisController();
        $rediss=$redis->connectredis();
        $component_access_token=$rediss->get('wechat:component_access_token');
        if (empty($component_access_token)){//如果token不存在，则重新获取
            $component_access_token=$this->get_component_access_token($this->getWechatComponentVerifyTicket());
        }
        ...
        if (is_json($code)){
            ...
            if (!array_key_exists('errcode',$code)){
                $rediss->set('wechat:pre_auth_code',$code['pre_auth_code'],array('ex'=>$code['expires_in']));
//                 echo $code['pre_auth_code'];
    ...
    public function getauthorizer(){
        ...
        $rediss->set('wechat:auth_code',$_GET['auth_code'],$_GET['expires_in']-20);
        ...
        if (is_json($return)){
            ...
            if ($savedata !== false){
                ...
                $rediss->set('wechat:authorizer_access_token:'.$array['authorization_info']['authorizer_appid'],$array['authorization_info']['authorizer_access_token'],$array['authorization_info']['expires_in']);
                //设置刷新token,很重要，微信方只给一次，一次性的!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!必须同时保存到mysql和redis
                $rediss->set('wechat:authorizer_refresh_token:'.$array['authorization_info']['authorizer_appid'],$array['authorization_info']['authorizer_refresh_token']);
                $back=$this->get_authorizer_info($array['authorization_info']['authorizer_appid']);
                $rediss->del('wechat:pre_auth_code');//此code只能使用一次
                if ($back==true)
    ...
    public function get_authorizer_info($authorizer_appid){
        ...
        if (is_json($return)){
            ...
            //目前的json没有对哪一个字段可以确定是公众号，如果未来腾讯再出新产品，将不能准确判断是哪一种产品，但小程序是有特有字段的，所以先判断是否是小程序
            if ( isset($array['authorizer_info']['MiniProgramInfo']) ) {//小程序
                ...
                $data['requestdomain'] = implode(',', $array['authorizer_info']['MiniProgramInfo']['network']['RequestDomain']);
                $data['wsrequestdomain'] = implode(',', $array['authorizer_info']['MiniProgramInfo']['network']['WsRequestDomain']);
                $data['uploaddomain'] = implode(',', $array['authorizer_info']['MiniProgramInfo']['network']['UploadDomain']);
                $data['downloaddomain'] = implode(',', $array['authorizer_info']['MiniProgramInfo']['network']['DownloadDomain']);
            ...
            }elseif (!isset($array['MiniProgramInfo'])) {
                $data['nick_name']=$array['authorizer_info']['nick_name'];
                $data['head_img']=$array['authorizer_info']['head_img'];
                $data['service_type_info']=$array['authorizer_info']['service_type_info']['id'];
                $data['verify_type_info']=$array['authorizer_info']['verify_type_info']['id'];
    ...
    public function authorizer_refresh_token($authorizer_appid){
        ...
        $params['component_appid']=$this->appId;
        $params['authorizer_appid']=$authorizer_appid;
        $params['authorizer_refresh_token']=$rediss->get('wechat:authorizer_refresh_token:'.$authorizer_appid);//获取这个授权方的刷新token
        ...
        if (is_json($return)){
            ...
            writeOperationLog($log,'authorizer_access_token');
            $array=json_decode($return,true);
            if (isset($array['errcode'])){
                return false;
            }else{
                //设置authorizer_access_token
                $rediss->set('wechat:authorizer_access_token:'.$authorizer_appid,$array['authorizer_access_token'],$array['expires_in']);
                //设置刷新token
                $rediss->set('wechat:authorizer_refresh_token:'.$authorizer_appid,$array['authorizer_refresh_token']);
    ...
    private function get_authorizer_option($option_name,$authorizer_appid){
        ...
        echo $rediss->get('wechat:component_access_token');
    ...
    public function component_access_token(){
        ...
        $component_access_token=$redis->get('wechat:component_access_token');
        if (empty($component_access_token)){//如果token不存在，则重新获取
            $component_access_token=$this->get_component_access_token($this->getWechatComponentVerifyTicket());
        }
    ...
    public function authorizer_access_token($appid){
        $rediss = new RedisController();
        $redis=$rediss->connectredis();
        $authorizer_access_token=$redis->get('wechat:authorizer_access_token:'.$appid);
