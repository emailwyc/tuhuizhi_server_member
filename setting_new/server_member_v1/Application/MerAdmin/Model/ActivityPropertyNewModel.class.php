...
class ActivityPropertyNewModel extends MSDaoBase{
    const  TABLENAME = 'integral_property_new';
    ...
    public function getAll($activity, $type_id, $vip_area, $buildid, $pid)
    {
        ...
        $res = $this->db->where($arr)->order('des ASC')->select();
    ...
    public function joinIntegralType($act_id, $admin_id, $buildid, $join)
    {
        ...
        if($admin_id != '')
        {
            $arr['integral_property_new.admin_id'] = $admin_id;
        }
        if($buildid != '')
        {
            $arr['buildid'] = $buildid;
