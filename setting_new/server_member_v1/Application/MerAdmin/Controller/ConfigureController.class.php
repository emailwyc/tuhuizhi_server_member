...
use MerAdmin\Service\ConsumptionScoreRuleService;
...
//            }
    ...
    public function SetSquared()
    {
        ...
        $classes = I('classes');
        $classes = empty($classes)?1:(int)$classes;
    ...
    public function GetSquaredList()
    {
        ...
        $where['classes']=array('eq',$classes);
    ...
    {
        $admininfo=$this->getMerchant($this->ukey);
        $find_birth=$this->GetOneAmindefault($admininfo['pre_table'], $this->ukey, 'birthday&idcardonly');
        
        //是否开启注册福利
        $find_status=$this->GetOneAmindefault($admininfo['pre_table'], $this->ukey, 'CreateIsWelfare');
        
        //属性配置
        $find_config=$this->GetOneAmindefault($admininfo['pre_table'], $this->ukey, 'AttributeConfiguration');
        
        //获取注册成功后跳转链接URL
        $find_url=$this->GetOneAmindefault($admininfo['pre_table'], $this->ukey, 'createsuccessurl');

        $scorerule = ConsumptionScoreRuleService::GetSconsumptionScoreRule($admininfo);
        if ($scorerule['code'] == 200){
            $this->redis->del('admin:default:one:sconsumptionscorerule'.$this->ukey);
        }

        
        //注册送券活动配置
        $find_activity=$this->GetOneAmindefault($admininfo['pre_table'], $this->ukey, 'ActivityCreateWelfare');
        
        //注册赠送积分配置
        $find_score=$this->GetOneAmindefault($admininfo['pre_table'], $this->ukey, 'ScoreCreateWelfare');

        $find_status['function_name']=$find_status['function_name']?$find_status['function_name']:1;
        $find_config['function_name']=$this->welfare_config($find_config['function_name'],'find');
        $data=array(
            'id_birth'=>$find_birth['function_name'],
            'status'=>$find_status['function_name'],
            'url'=>$find_url['function_name'],
            'activity'=>$find_activity['function_name'],
            'score'=>$find_score['function_name'],
            'config'=>$find_config['function_name'],
            'scorerule'=>$scorerule['data']
        );
    ...
    public function SetChangeConfig()
    {
        ...
        $params['isopen'] = (int)I('isopen');
        $params['scorenum'] = I('scorenums');


        if ($params['isopen'] == 1) {
            foreach ($params['scorenum'] as $key => $value){
                if (!isset($value['code'])) {
                    returnjson(array('code'=>1051, 'data'=>4), $this->returnstyle, $this->callback);
                }
        ...
        if ($params['isopen'] != 1 && $params['isopen'] != 0) {
            returnjson(array('code'=>1051, 'data'=>4), $this->returnstyle, $this->callback);
        }
        $set = ConsumptionScoreRuleService::SetSconsumptionScoreRule($params['isopen'], $params['scorenum'], $admininfo);
        if ($set['code'] == 200){
            $this->redis->del('admin:default:one:sconsumptionscorerule:'.$this->ukey);
        }
        ...
        $params['url']=I('url')?I('url'):'';
        $params['activity']=I('activity')?I('activity'):'';
        $params['score']=I('score')?I('score'):'';
        $params['config']=I('config')?I('config'):'';
        
        $status=$this->GetOneAmindefault($admininfo['pre_table'], $this->ukey, 'CreateIsWelfare');
        
        if($status['function_name'] == 2){//开启状态才让修改
            if( $params['config'] != '' ){//属性配置
                if($params[$params['config']] == ''){
                    returnjson(array('code'=>1030), $this->returnstyle, $this->callback);exit;
                }
            }
            $params['url'] = $params['url']?html_entity_decode($params['url']):'';
            $res=$this->funciton_action_save($this->ukey,$admininfo['pre_table'],$params['url'],'createsuccessurl','设置注册成功之后跳转地址');
            
            $params['activity'] = $params['activity']?html_entity_decode($params['activity']):'';
            $res1=$this->funciton_action_save($this->ukey,$admininfo['pre_table'],$params['activity'],'ActivityCreateWelfare','设置注册活动配置');
            
            $params['score'] = $params['score']?html_entity_decode($params['score']):'';
            $res2=$this->funciton_action_save($this->ukey,$admininfo['pre_table'],$params['score'],'ScoreCreateWelfare','设置注册赠送积分配置');
            
            if( $params['config'] != '' ){//属性配置
                
                $config=$this->welfare_config($params['config'],'save');
                
                $res=$this->funciton_action_save($this->ukey,$admininfo['pre_table'],$config,'AttributeConfiguration','注册成功属性配置');
                if($res === false){
    ...
    public function welfare_config($config , $status){
    
        $arr=array(
            'url'=>'createsuccessurl',
            'activity'=>'ActivityCreateWelfare',
            'score'=>'ScoreCreateWelfare',
        );
    
        if($status=='find'){
            if($config != ''){
                return array_search($config, $arr);
            }else{
                return 'no';
            }
        }else if($status=='save'){
    ...
    public function funciton_action_save($key_admin,$table,$value,$name,$desc){
    
        $db=M('default', $table);
    
        $find=$db->where(array('customer_name'=>$name))->find();
        if ($find){//数据存在则修改
            $res=$db->where(array('customer_name'=>$name))->save(array('function_name'=>$value));
    
            $this->redis->del('admin:default:one:'.$name.':'.$key_admin);//删除redis缓存
        }else{//反之添加
            $res=$db->add(array('customer_name'=>$name,'function_name'=>$value, 'description'=>$desc));
        }
        return $res;
    }
     ...
     * 是否开启注册福利
     */
    public function IsOpenWelfare(){
        $admininfo=$this->getMerchant($this->ukey);
        $find_status=$this->GetOneAmindefault($admininfo['pre_table'], $this->ukey, 'CreateIsWelfare');
    
        $db=M('default',$admininfo['pre_table']);
        if(!$find_status){
            $find_status['function_name']=2;
            $return=$db->add(array('customer_name'=>'CreateIsWelfare','function_name'=>$find_status['function_name'],'description'=>'是否开启注册福利'));
    
            $returns=$db->add(array('customer_name'=>'AttributeConfiguration','function_name'=>'no','description'=>'注册福利属性配置'));
        }else{
            $find_status['function_name']=$find_status['function_name']==1?2:1;
    
            $return=$db->where(array('customer_name'=>'CreateIsWelfare'))->save(array('function_name'=>$find_status['function_name']));
            $this->redis->del('admin:default:one:CreateIsWelfare:'. $this->ukey);//删除redis缓存
        }
    
        if($return){
            $msg['code']=200;
            $msg['data']['status']=$find_status['function_name'];
        }else{
            $msg['code']=104;
        }
        returnjson($msg, $this->returnstyle, $this->callback);
  ...
  PRIMARY KEY (`id`)
            ...
            $sql = "CREATE TABLE `".$admininfo['pre_table']."form_default` (
...
//            returnjson(array('code' => 102, 'data' => array('form' => $sel, 'mylist' => $selbussiness)), $this->returnstyle, $this->callback);
  ...
  PRIMARY KEY (`id`)
    ...
    public function delQuesGroup()
    {
        ...
        $re = $wj->where(array('groupId' => $id,'adminId'=>$merchant['id']))->find();
...
//     }
    ...
    public function save_carousel(){
        ...
        if(in_array('', $params)){
            ...
            if($id){
                $res=$db->where(array('id'=>$id))->save($params);
            }else{
                $merchant = $this->getMerchant($this->ukey);
                $params['admin_id']=$merchant['id'];

                $params['sort']=$params['sort']?$params['sort']:1;
                $res=$db->add($params);
            }
     ...
     *获取会员签到开关设置(1:打开，2:关闭)
     */
    public function SetSignSwitch()
    {
        $isenable = I('isenable');
        $isenable = (!empty($isenable) && $isenable==2)?2:1;
        $this->redis->del("admin:default:one:isshowsignbutton:$this->ukey");
        $db = M('default', $this->admin_arr['pre_table']);
        $sel=$db->where(array('customer_name'=>'isshowsignbutton'))->find();
        if ($sel){
            $save=$db->where(array('customer_name'=>'isshowsignbutton'))->save(array('function_name'=>$isenable));
        }else{
            $save=$db->add(array('customer_name'=>'isshowsignbutton','function_name'=>$isenable,'description'=>"签到按钮开关"));
        }
        $this->redis->del("admin:default:one:isshowsignbutton:$this->ukey");
        returnjson(array('code'=>200), $this->returnstyle, $this->callback);
    }
     ...
     * 配置消费积分使用规则
     */
    public function setconsumptionscorerule()
    {
        $params['isopen'] = (int)I('isopen');
        $params['scorenum'] = I('scorenums');
     ...
     * 获取消费积分使用规则
     */
    public function getconsumptionscorerule()
    {
        $admininfo = $this->getMerchant($this->ukey);
        $data = ConsumptionScoreRuleService::GetSconsumptionScoreRule($admininfo);
        if ($data['code'] == 200){
            $this->redis->del('admin:default:one:sconsumptionscorerule'.$this->ukey);
        }
        returnjson($data, $this->returnstyle, $this->callback);
