...
// 		$params['collectaddress']=I('collectaddress');//收件地址
		
		$params['name']=replace_specialChar($params['name']);//过滤特殊字符
		$params['address']=replace_specialChar($params['address']);//过滤特殊字符
		$params['career']=replace_specialChar($params['career']);//过滤特殊字符
		$params['remark']=replace_specialChar($params['remark']);//过滤特殊字符
		$params['collectaddress']=replace_specialChar($params['collectaddress']);//过滤特殊字符
		
		if($params['key_admin'] == '75458833a43dc64df5069e03bdad1ec5'){
		    if($params['idnumber'] !=''){
		        if(!isIdCard($params['idnumber'])){
		            $msg['code']=1061;
		            echo returnjson($msg,$this->returnstyle,$this->callback);
		        }else{
		            $births=getIDCardInfo($params['idnumber']);
		        }
		    }

		    if($params['birth'] == ''){
		        $params['birth']=strtotime($births['birthday'])?strtotime($births['birthday']):'';
		    }
		    
		    if($params['star'] == ''){
		        if($params['birth'] != ''){
		            $params['star']=getConstellation(date('m',$params['birth']),date('d',$params['birth']));
		        }else{
		            $params['star']='';
		        }
		    }
...
// 			print_r($Ay_db);die;
	...
	protected function mem_action($params,$admininfo){
	    $db=M('mem',$admininfo['pre_table']);
	    $find=$this->GetOneAmindefault($admininfo['pre_table'], $params['key_admin'], 'member_level');
	    $arr=json_decode($find['function_name'], true);
	    $level_code = empty($arr)?'':$arr[0];
	    ...
	    $data['idnumber'] = (null==$params['idnumber'])?'':$params['idnumber'];
	    $data['getcarddate']=date('Y-m-d H:i');
	    $data['phone']=$params['mobile'];
	    $data['mobile']=$params['mobile'];
	    $data['birthday']=(null==$params['birth'])?'':$params['birth'];
	    $data['sex']=(null==$params['sex'])?'':$params['sex'];
	    $data['score_num']=0;
	    $data['is_del']=I('is_del')!=''?I('is_del'):1;
	    $data['address']=(null==$params['address'])?'':$params['address'];
	    $data['wechat']=(null==$params['wechat'])?'':$params['wechat'];
	    $data['career']=(null==$params['career'])?'':$params['career'];
	    $data['star']=(null==$params['star'])?'':$params['star'];
	    $data['remark']=(null==$params['remark'])?'':$params['remark'];
	    $data['email']=(null==$params['email'])?'':$params['email'];
	    $data['province']=(null==$params['province'])?'':$params['province'];
	    $data['city']=(null==$params['city'])?'':$params['city'];
	    $data['district']=(null==$params['district'])?'':$params['district'];
	    $data['level']=!empty($params['level'])?$params['level']:$level_code;
	    $data['collectaddress']=(null==$params['collectaddress'])?'':$params['collectaddress'];//收件地址
			...
			if(!empty($mem_arr)){
				if($mem_arr['cardno']==$params['cardno']){
					...
					if($params['collectaddress'] !=''){
					    $rt['collectaddress']=$params['collectaddress'];//收件地址
					}
...
// 			}else{
				...
				if($mem_data){
				    if($mem_data['is_del']!=1){
                        ...
                        $arr['collectaddress']=$mem_data['collectaddress'];
				...
				if($res){
                    ...
                    if (is_array($getone) && $getone['function_name'] == 1){
                        ...
                        if (is_array($getnums) && $getnums['function_name'] >0){
                            ...
                            $historyscore=$dbscore->where(array('cardno'=>$params['cardno'],'cutadd'=>2))->count();//数量，只要有一条数据，则执行代码
                            if (1 == $historyscore){
                                $db=M('mem',$admininfo['pre_table']);
