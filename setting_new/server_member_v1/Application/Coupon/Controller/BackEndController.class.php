...
//                 if (empty($v['city'])) {
                    ...
                    if (is_array($details)) {
                        ...
                        $data['url'] = $result[$k]['url']?$result[$k]['url']:$details['url'];
  ...
  PRIMARY KEY (`id`)
    ...
    public function property_list(){
        ...
        if(in_array('', $params)){
            ...
            $cou_arr=$db->where(array('buildid' => $params['buildid'], 'admin_id' => $this->merchant['id'],'is_status'=>1))->select();
//             print_r($cou_arr);
            if($cou_arr){
...
//                 print_R($cou_id);die;
                if(!empty($cou_id) && !$params['status']){
                    $cou_id_str=implode(',', $cou_id);
                    $db->where(array('id' => array('in',$cou_id_str)))->save(array('is_status'=>2));
//                     echo $db->_sql();
                }
            }
            $msg['code']=200;
    ...
    public function coupon_save(){
        ...
        if(in_array('', $params)){
            ...
            $cou_arr=$db->where(array('buildid' => $params['buildid'], 'admin_id' => $this->merchant['id'],'pid'=>$params['pid'],'is_status'=>1))->find();
            ...
            if($cou_arr){
                ...
                $res=$db->where(array('id'=>$cou_arr['id']))->save($data);
