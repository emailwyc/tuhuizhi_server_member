...
class XianjindiCrmController extends CommonController implements CrminterfaceController
{
    ...
    protected $request_url = 'http://6242.test.hd123.cn:8180/jcrm-server-card/rest';//测试地址
    ...
    public function scorelist(){
        ...
        $page = !empty($params['page'])?(int)abs($params['page']):1;
        $offset = !empty($params['lines'])?(int)$params['lines']:20;
        $admininfo=$this->getMerchant($params['key_admin']);
        $start = ($page-1)*$offset;
        $db=M('score_record',$admininfo['pre_table']);
        $sT = !empty($params['startdate'])?date('Y-m-d H:i:s',$params['startdate']):"0";
        $eT = !empty($params['enddate'])?date('Y-m-d H:i:s',$params['enddate']):date('Y-m-d H:i:s',time());
        $where = array('datetime'=>array('lt',$eT),'datetime'=>array('gt',$sT));
