...
//         }
    ...
    public function member_feedback(){
        ...
        $number=$db->where($map)->join($member_table.' on '.$feedback_table.'.mem_id = '.$member_table.'.id')->field($feedback_table.'.id')->select();
//        echo $db->_sql();die;
        $num=count($number);
        if($num){
            //echo $num;die;
            ...
            $return=$db->join($member_table.' on '.$feedback_table.'.mem_id = '.$member_table.'.id')->where($map)->order('createtime desc')->field($feedback_table.'.openid,'.$feedback_table.'.id,'.$feedback_table.'.status,'.$feedback_table.'.phone,'.$feedback_table.'.content,'.$feedback_table.'.createtime,'.$member_table.'.usermember')->limit($end,$lines)->select();
...
//         print_r($msg);
    ...
    public function FeedbackReplayList()
    {
        ...
        if ($feedbackid != null){
            ...
            $mem=$db->where(array('openid'=>$feedback[0]['openid']))->find();
            if ($mem != null){
                foreach ($feedback as $key => $value) {
                    if ($value['openid'] != ''){
                        $feedback[$key]['usermember']=$mem['usermember'];
                    }
                }
            }
            $data['code']=200;
