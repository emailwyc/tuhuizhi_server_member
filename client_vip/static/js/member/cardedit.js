require('../../scss/member/cardedit.scss');
require('../bootstrap/modal');
import { out } from '../modules/out.js';
import { saveMemberCode, getUploadToken, onceMemberCode } from '../model/index';
const $ = window.$;
...
const cardedit = {
  ...
  initDom() {
    ...
    // console.log(this.state.ctg.imgurl);
  },
  initEvent() {
    this.$out.on('click', () => {
      out();
  ...
  uploadImage() {
    saveMemberCode({
      ...
      imgurl: this.file || this.$image.attr('src'),
      id: conf.id || '',
  ...
  onceMemberCode() {
    onceMemberCode({
      ...
      console.log(json);
      this.$cardnumber.val(json.data.code);
      this.$cardname.val(json.data.name);
      this.$sort.val(json.data.sort);
      this.$image.attr('src', json.data.imgurl);
      $('.register').find(`input[name="register"][value="${json.data.is_register}"]`)
      .attr('checked', true);
      $('.default').find(`input[name="default"][value="${json.data.is_default}"]`)
      .attr('checked', true);
    });
  },
};
